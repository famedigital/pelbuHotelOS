...
/** Conservative by default. Set PELBU_PAID_TIER=1 (or NEXT_PUBLIC_PELBU_PAID_TIER=1) when paid. */
...
/**
 * Public hosts that always map to flagship Pelbu — no Supabase lookup in middleware.
 * Keep in sync with production domains.
 */
const FLAGSHIP_HOSTS = new Set([
  "pelbusuites.bt",
  "www.pelbusuites.bt",
  "app.pelbusuites.bt",
  "desk.pelbusuites.bt",
...
export function isFlagshipHost(hostHeader: string | null): boolean {
  ...
  if (FLAGSHIP_HOSTS.has(host)) return true;
  // Preview deployments: still single-property flagship
  if (host.endsWith(".vercel.app")) return true;
  const extra = process.env.FLAGSHIP_HOSTS?.split(",") ?? [];
  return extra.some((h) => h.trim().toLowerCase() === host);
}

/** Optional UUID so middleware never hits DB for flagship. Set in Vercel env when known. */
...
export function flagshipSlug(): string {
  return process.env.FLAGSHIP_PROPERTY_SLUG?.trim() || PELBU_PROPERTY_SLUG;
}
