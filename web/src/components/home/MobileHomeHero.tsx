...
/**
 * Phone-first homepage hero: portrait art is primary; booking opens as a
 * bottom sheet so the photo is not buried under the full search form.
 */
export function MobileHomeHero({
  ...
  return (
    <section
      className="relative isolate md:hidden"
      style={{ backgroundColor: theme.scrimBottom }}
    >
      {/* Portrait-first media frame — photo is the star of the first viewport. */}
      <div className="relative min-h-[70svh] max-h-[82svh] w-full overflow-hidden">
        ...
        {/* Light bottom scrim only — keep mid/top of portrait clear. */}
        <div
          className="pointer-events-none absolute inset-x-0 bottom-0 h-[42%]"
          style={{
            background: `linear-gradient(to top, ${hexAlpha(theme.scrimBottom, 0.88)} 0%, ${hexAlpha(theme.scrimBottom, 0.35)} 55%, transparent 100%)`,
          }}
          aria-hidden
        />

        <div className="relative flex min-h-[70svh] max-h-[82svh] flex-col justify-end px-5 pb-6 pt-20">
          <motion.div
            {...settle}
      ...
      {/* Space for the fixed book dock. */}
      <div
        className="h-[calc(4.75rem+env(safe-area-inset-bottom))]"
        aria-hidden
      />

      {/* Slim book dock — does not cover the hero photo. */}
      ...
      <Sheet open={bookOpen} onOpenChange={setBookOpen}>
        <SheetContent
          side="bottom"
          className="max-h-[90dvh] rounded-t-2xl border-border px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-2"
        >
