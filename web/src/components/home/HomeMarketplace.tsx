...
export function HomeMarketplace({ dishCount }: { dishCount: number }) {
  return (
    <section id="marketplace" className="px-5 py-16 md:px-8 md:py-24">
      <div className="relative mx-auto max-w-[1200px] overflow-hidden rounded-[2rem] bg-gradient-to-br from-juniper via-juniper to-juniper-soft px-6 py-14 shadow-[0_40px_100px_-50px_rgba(18,26,23,0.55)] md:px-14">
        <div
          className="absolute -right-16 -top-16 size-[22rem] rounded-full bg-ember/30 blur-[110px]"
          aria-hidden
        />
        <div
          className="absolute -bottom-24 -left-10 size-[20rem] rounded-full bg-juniper/30 blur-[110px]"
          aria-hidden
        />
        ...
        <ul className="relative mt-10 grid gap-4 md:grid-cols-3">
          {STEPS.map((step) => {
            ...
            return (
              ...
              >
                <span className="inline-flex size-10 items-center justify-center rounded-xl bg-gradient-to-br from-ember-soft to-ember text-cedar-ink">
        ...
        <div className="relative mt-10 flex flex-wrap gap-3">
          <Link
            href="/menu"
            className="inline-flex h-12 items-center rounded-xl bg-gradient-to-r from-ember-soft to-ember px-6 text-sm font-semibold text-cedar-ink shadow-[0_16px_40px_-16px_rgba(245,158,11,0.9)] transition-transform motion-safe:hover:-translate-y-0.5"
          >
            Open the menu
          </Link>
          <Link
