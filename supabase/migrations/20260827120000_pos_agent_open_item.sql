-- POS open item: travel-agent F&B with no stay — invoice now, collect later.
...
comment on column folios.agent_id is
  'Bill-to travel agent for city-ledger / POS open-item (no stay) folios.';

comment on column orders.agent_id is
  'Travel agent charged on POS settle when lunch is invoiced later (no room stay).';

create index if not exists folios_agent_idx
  on folios (property_id, agent_id, status)
  where agent_id is not null;

create index if not exists folios_walk_in_idx
  on folios (property_id, folio_type, status)
  where folio_type = 'walk_in';

create index if not exists orders_agent_idx
  on orders (property_id, agent_id)
  where agent_id is not null;

-- Existing hotels: collect cash/bank/card against agent AR (not guest AR).
...
cross join (
  values
    ('payment.cash.agent', 'cash', 'ar_agent', null::text),
    ('payment.bank.agent', 'bank', 'ar_agent', null::text),
    ('payment.card.agent', 'card_clearing', 'ar_agent', null::text)
...
begin
  ...
  values
    ...
    (p_property_id, 'payment.cash.agent', 'cash', 'ar_agent', null),
    (p_property_id, 'payment.bank.agent', 'bank', 'ar_agent', null),
    (p_property_id, 'payment.card.agent', 'card_clearing', 'ar_agent', null),
