-- Comp folio credits: DR contra-revenue / CR guest AR (positive amount).

insert into accounting_posting_rules (property_id, event_type, debit_system_key, credit_system_key, department_code)
select p.id, 'folio_line.comp', 'rev_other', 'ar_guest', 'ADMIN'
from properties p
on conflict (property_id, event_type) do nothing;

-- Extend accounting_seed_property for future properties.
...
begin
  ...
  values
    ...
    (p_property_id, 'folio_line.comp', 'rev_other', 'ar_guest', 'ADMIN'),
