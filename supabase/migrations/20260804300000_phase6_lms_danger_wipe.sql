...
begin
  ...
  -- Laundry custody (before orders/bookings)
  delete from laundry_bag_events lbe
  using laundry_order_bags lob, laundry_orders lo
  where lbe.bag_id = lob.id and lob.laundry_order_id = lo.id
    and lo.property_id = p_property_id;
  get diagnostics v_n = row_count;
  v_counts := v_counts || jsonb_build_object('laundry_bag_events', v_n);

  delete from laundry_bag_items lbi
  using laundry_order_bags lob, laundry_orders lo
  where lbi.bag_id = lob.id and lob.laundry_order_id = lo.id
    and lo.property_id = p_property_id;

  delete from laundry_order_bags lob
  using laundry_orders lo
  where lob.laundry_order_id = lo.id and lo.property_id = p_property_id;

  delete from laundry_order_events loe
  using laundry_orders lo
  where loe.laundry_order_id = lo.id and lo.property_id = p_property_id;

  delete from laundry_order_items loi
  using laundry_orders lo
  where loi.laundry_order_id = lo.id and lo.property_id = p_property_id;
